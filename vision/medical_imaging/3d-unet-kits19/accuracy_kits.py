...
def evaluate(target_files: List,
    ...
    for uniq_case in target_files:
        case = uniq_case.split("^dup")[0]
        ...
        prediction_path = Path(postprocessed_data_dir, "KiTS19", "prediction",
                               "nifti", uniq_case, "prediction.nii.gz").absolute()
        ...
        assert groundtruth.shape == prediction.shape,\
            "{}({}) -- groundtruth: {} and prediction: {} have different shapes".format(
                uniq_case, case, groundtruth.shape, prediction.shape)

        bundle.append((uniq_case, groundtruth, prediction))
...
def save_predictions(predictions: Dict,
    ...
    bundle = list()
    for uniq_case, case_d in predictions.items():
        case = case_d['case']
        pred_file_path = Path(output_dir, "KiTS19", "prediction", "nifti", uniq_case, "prediction.nii.gz")
...
def load_loadgen_log(log_file: str,
    ...
    if len(predictions) != len([k for k in aux.keys() if k in file_list]):
        print("Warning: the number of predictions does not match to the total number of samples in validation set.")

    results = dict()
    # if multiple accuracy data are logged for the same case number, due to audit test, give unique case number
    uniq_case_cnt = dict()
    for prediction in predictions:
        ...
        if case in uniq_case_cnt:
            uniq_case = uniq_case + "^dup{}".format(uniq_case_cnt[case])
            uniq_case_cnt[case] += 1
        else:
            uniq_case_cnt[case] = 1
